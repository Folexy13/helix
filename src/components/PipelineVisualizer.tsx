...
export default function PipelineVisualizer() {
  ...
  const styledNodes = useMemo(() => {
    return nodes.map((node) => {
      ...
      return {
        ...node,
        style: {
          background: isWaiting ? '#f97316' : isProcessing ? '#3b82f6' : '#1e293b',
          color: '#f8fafc',
          border: isWaiting ? '2px solid #ea580c' : isProcessing ? '2px solid #60a5fa' : '1px solid #334155',
          borderRadius: '8px',
          padding: '10px 20px',
          fontWeight: isProcessing || isWaiting ? 'bold' : 'normal',
          boxShadow: isProcessing ? '0 0 15px rgba(59, 130, 246, 0.5)' : isWaiting ? '0 0 15px rgba(249, 115, 22, 0.5)' : 'none',
  ...
  return (
    <div style={{ width: '100%', height: '100%' }} className="bg-background rounded-xl overflow-hidden border border-border">
      <ReactFlow
        nodes={styledNodes}
        edges={edges}
      ...
      >
        <Controls />
        <MiniMap nodeStrokeWidth={3} nodeColor="#334155" maskColor="#0f172a" />
